...
const BookmarkList: React.FC<BookmarkListProps> = ({
  ...
  return (
    <div className="p-2">
      <h3 className="label-mono mb-3 text-muted">{t('chat_bookmarks_header')}</h3>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {bookmarks.map(bookmark => (
          <div
            ...
            className="glass-card group relative p-3 transition-opacity hover:opacity-80">
            {editingId === bookmark.id ? (
              <div className="flex items-center">
                <input
                  ...
                  className="glass-input mr-2 grow px-2 py-1 text-sm text-ink"
                />
                <button
                  onClick={() => handleSaveEdit(bookmark.id)}
                  className="rounded p-1 text-green-500 transition-opacity hover:opacity-80"
                ...
                <button
                  ...
                  className="ml-1 rounded p-1 text-red-500 transition-opacity hover:opacity-80"
                  style={{ background: 'var(--surface)' }}
              ...
              <>
                <div className="flex items-center">
                  <button
                    ...
                    <div className="truncate pr-10 text-sm font-medium text-ink">{bookmark.title}</div>
                  </button>
                </div>
              </>
            )}
            ...
            {editingId !== bookmark.id && (
              <>
                ...
                <button
                  ...
                  className="absolute right-[28px] top-1/2 z-10 -translate-y-1/2 rounded p-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100 text-amber"
                  style={{ background: 'var(--surface)' }}
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 z-10 -translate-y-1/2 rounded p-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100 text-muted"
                  style={{ background: 'var(--surface)' }}
